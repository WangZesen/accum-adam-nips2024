...
from data import get_datasets, get_dataloaders, DistributedTokenBatchSampler
...
def ar_train_epoch(cfg: Config,
    ...
    for batch_idx, (src, tgt, n_tokens) in enumerate(train_ds):
        ...
        _fw_start = torch.cuda.Event(enable_timing=True)
        _fw_start.record()
        _fw_start.synchronize()

        if fw_start is not None:
            fw_end.synchronize()
            fw_times.append(fw_start.elapsed_time(fw_end))
    ...
    indices = sampler.get_local_indices()[10:]
    forward_times = fw_times[10:]

    indices = indices[:min(len(forward_times), len(indices))]
    forward_times = forward_times[:min(len(forward_times), len(indices))]

    stats_res = gather_time_stats(forward_times, indices)
    sampler.update_time_stats(stats_res)
...
def decent_train_epoch(cfg: Config,
    ...
    indices = sampler.get_local_indices()[10:]
    forward_times = list(model.get_time_stats()['forward'])[10:]
